...
router.post('/wave/webhook', express.raw({ type: 'application/json' }), async (req, res) => {
  try {
    ...
    const isValid = waveCheckoutService.verifyWebhookSignature({
      signature,
      rawBody,
    });
    ...
    console.log('✅ Webhook valide:', event);

    // Exemple traitement paiement
    const sessionId =
      event?.id ||
      event?.session_id ||
      event?.data?.id;
    ...
    if (sessionId) {
      console.log('💰 Paiement reçu:', sessionId);
    }
