...
// ==========================================
// CORIS RETRAITE - Tarifs (capital pour prime de référence)
// ==========================================
async function migrateRetraite() {
  console.log('Migrating CORIS RETRAITE data...');

  // Prime de référence pour le calcul des capitaux (mêmes valeurs que dans l'application)
  ...
  // Capitaux correspondants à la prime de référence pour chaque durée et périodicité
  const capitalValues = {
    5: {
  ...
  try {
    ...
    for (const [duree, periodicites] of Object.entries(capitalValues)) {
      for (const [periodicite, capital] of Object.entries(periodicites)) {
        tarifs.push({
          produit_id: produitId,
          duree_contrat: parseInt(duree),
          periodicite: periodicite,
          prime: primeReferenceValues[periodicite],
          age: null,
